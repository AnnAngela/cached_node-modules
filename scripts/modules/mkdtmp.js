...
export default async () => {
    ...
    // 前缀不以 X 结尾：Node 文档要求避免 prefix 含尾随 X（部分平台会将其当作随机位替换，
    // 触发 "templates ending with X are not portable" 警告）。让 mkdtemp 自行追加随机字符。
    const prefix = join(baseDir, "cached_node-modules-");
