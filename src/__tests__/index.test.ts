...
const setupFiles = (pm: string, lockfilePath: string) => {
    vol.reset();
    const lockContent = pm === "npm"
        ? JSON.stringify({
            lockfileVersion: 3,
...
const resetState = () => {
    vi.resetModules();
    vi.clearAllMocks();
    Object.keys(mockCoreInputs).forEach((k) => Reflect.deleteProperty(mockCoreInputs, k));
    Object.keys(mockCoreStates).forEach((k) => Reflect.deleteProperty(mockCoreStates, k));
    Object.keys(mockCoreOutputs).forEach((k) => Reflect.deleteProperty(mockCoreOutputs, k));
