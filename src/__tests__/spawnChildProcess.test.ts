...
import type { ExecFileCallback } from "./testTypes.js";
...
describe("spawnChildProcess", () => {
    ...
    describe("network error retry", () => {
        it("should retry on ECONNREFUSED error with npm prefix", async () => {
            let callCount = 0;
            mockExecFile.mockImplementation(
                (_cmd: string, _args: string[], _opts: unknown, cb: ExecFileCallback) => {
                    callCount++;
        ...
        it("should retry on npm internal error (Exit handler never called!)", async () => {
            let callCount = 0;
            mockExecFile.mockImplementation(
                (_cmd: string, _args: string[], _opts: unknown, cb: ExecFileCallback) => {
                    callCount++;
                    if (callCount === 1) {
                        cb(new Error("command failed"), "", "npm error Exit handler never called!");
                    } else {
                        cb(null, "success after retry", "");
                    }
                },
            );

            vi.useFakeTimers();
            const promise = execCommand("npm ci", { cwd: "/", retryTime: 1 });
            await vi.runAllTimersAsync();
    ...
    describe("stdout/stderr piping", () => {
        it("should pipe stdout when synchronousStdout is true", () => {
            ...
            void execCommand("test", { cwd: "/", synchronousStdout: true });
        ...
        it("should pipe stderr when synchronousStderr is true", () => {
            ...
            const mockChild = {
                stdout: {},
                stderr: { pipe: mockPipe },
            };

            mockExecFile.mockReturnValue(mockChild);

            void execCommand("test", { cwd: "/", synchronousStderr: true });
