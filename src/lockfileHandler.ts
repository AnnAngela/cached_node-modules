...
export const yarnBerryLockHandler = async (lockfilePath: string, lockfileParsedPath: path.ParsedPath, content?: string) => {
    ...
    // The header comment line "# This file is generated by running..."
    // is stripped implicitly by YAML.parse(), which discards YAML comments
    // at the representation level (they are not part of the data model).
    // If the code is ever refactored to use YAML.parseDocument() for CST
    // access, the header comments must be handled explicitly.
    return { tmpdir, lockfileContent: YAML.stringify(parsed) };
};
